...
## Loading the `uniprot_region` dataset into RStudio {.build}
...
devtools::install_github("BAREJAA/Tidybio_pkg", build_vignettes = TRUE, 
                         force = TRUE)
...
## Converting the `uniprot_region` dataset to a tbl {.build}
...
## Inspecting the `uniprot` dataset {.build}
...
uniprot %>% 
  select(Protein.Length, Gene.Start:Gene.Length)
```
...
uniprot %>% 
  mutate(gene_length_1000 = Gene.Length/1000)
```
...
uniprot %>% 
  summarise(protein_mean = mean(Protein.Length), 
            protein_median = median(Protein.Length),
            protein_sd = sd(Protein.Length))
```
