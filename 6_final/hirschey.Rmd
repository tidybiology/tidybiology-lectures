...
#variation EDA
chromosome %>% mutate(percent_var = variations/basepairs) %>% select(chromosome, percent_var) %>% arrange(percent_var)
...
#xy var
x_var <- chromosome %>% filter(chromosome == "X") %>% select(percent_var) %>% pull()
y_var <- chromosome %>% filter(chromosome == "Y") %>% select(percent_var) %>% pull()
...
#need a different arrow set for negative curvature
arrows2 <- tibble(
  x1 = c(.02),
  x2 = c(mean_variation),
  y1 = c(18.5),
...
ggplot(chromosome, aes(x = percent_var, y = fct_rev(chromosome))) +
  ...
  geom_mark_ellipse(aes(filter = chromosome == "Y", 
  ...
  annotate("text", x = 0.014, y = 3.2, size = 4, color = "gray60",
  ...
  geom_curve(data = arrows2, aes(x = x1, y = y1, xend = x2, yend = y2),
             arrow = arrow(length = unit(0.07, "inch")), size = 0.4,
             color = "gray60", curvature = -0.3) +
  geom_text(aes(label = fct_rev(chromosome)), color = "white", size = 3, fontface = "bold") +
