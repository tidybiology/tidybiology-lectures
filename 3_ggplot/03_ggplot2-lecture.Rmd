...
```{r, include=FALSE}
load("uniprot_region.rda") #this is a dataset that Akshay made for this class
data(chromosome)
```
...
Question- without running it, predict what the following code will do:
...
2. size - changes the size of your data point
3. shape - sets the shape of a point to one of the pre-defined shape classes
4. alpha - changes the transparency of each point
5. fill- changes the fill color of your graph
...
example:
```{r}
ggsave("ggsaveexample.pdf", width = 6, height = 6, units = "in")
