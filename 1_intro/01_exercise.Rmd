...
exp(1)^(pi * sqrt(-1)) #this is a better example, but R freaks out; sorry Euler
```
...
You'll learn more about R Markdown throughout our time together, and all of your coding and asssignments will take place in R markdown documents.
...
```{r}
# ggplot(data = chromosome) + geom_point(aes(x = id, y = basepairs))
```
